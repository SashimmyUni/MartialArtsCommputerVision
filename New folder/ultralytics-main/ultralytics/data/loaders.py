...
def get_best_youtube_url(url: str, method: str = "pytube") -> str | None:
    ...
    if method == "pytube":
        ...
        # Sort descending by resolution; guard against None resolution values
        streams = sorted(
            streams,
        ...
        for stream in streams:
            if stream.resolution and int(stream.resolution[:-1]) >= 1080:  # prefer at least 1080p
                return stream.url
        # Fall back to the best available resolution if no 1080p stream exists
        for stream in streams:
