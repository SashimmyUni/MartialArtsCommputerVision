...
def resolve_reference_path(args: argparse.Namespace) -> Path:
    if args.reference_file:
        p = _resolve_project_path(args.reference_file)
        if p.exists():
            return p
    ...
    p = _resolve_project_path(args.reference_dir) / args.technique / f"{args.angle}.npy"
    if not p.exists():
        raise FileNotFoundError(f"reference file not found: {p}")
    return p
...
def discover_reference_paths(reference_dir: str) -> list[Path]:
    root = _resolve_project_path(reference_dir)
    if not root.exists():
...
def render_reference(
    ...
    writer = None
    if save_video:
        out_path = _resolve_project_path(save_video)
...
def main() -> int:
    ...
    if args.all:
        ...
        out_root = _resolve_project_path(args.batch_output_dir)
